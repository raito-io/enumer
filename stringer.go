...
func (g *Generator) declareIndexAndNameVars(runs [][]Value, typeName string) {
	indexes := make([]string, 0, len(runs))
	names := make([]string, 0, len(runs)*2)
	for i, run := range runs {
		index, n := g.createIndexAndNameDecl(run, typeName, fmt.Sprintf("_%d", i))
		indexes = append(indexes, index)
		names = append(names, n)
		n = g.createLowerIndexAndNameDecl(run, typeName, fmt.Sprintf("_%d", i))
		names = append(names, n)
	}
	g.Printf("const (\n")
	for _, n := range names {
...
func (g *Generator) declareIndexAndNameVar(run []Value, typeName string) {
	...
	g.Printf("const %s\n", n)
	g.Printf("var %s\n", index)
	n = g.createLowerIndexAndNameDecl(run, typeName, "")
	g.Printf("const %s\n", n)
}

// createIndexAndNameDecl returns the pair of declarations for the run. The caller will add "const" and "var".
func (g *Generator) createLowerIndexAndNameDecl(run []Value, typeName string, suffix string) string {
	b := new(bytes.Buffer)
...
func (g *Generator) buildMap(runs [][]Value, typeName string) {
	...
	for _, values := range runs {
		for i, value := range values {
			g.Printf("\t%s: _%sName[%d:%d],\n", &values[i], typeName, n, n+len(value.name))
			n += len(value.name)
		}
	}
	g.Printf("}\n\n")
